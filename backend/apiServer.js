...
// ========== 2. 核心配置（E同学确认/替换以下配置） ==========
// 百度AI密钥（已填充你提供的真实值）
const BAIDU_API_KEY = '2gVFAVI89KSInO6YMCMGimSt';     
const BAIDU_SECRET_KEY = 'xqmPDVOjZIq3MbNjL3bJUuLCQTe2yAvC'; 
// 服务端口（避免和前端冲突）
const PORT = 3001;
// 跨域配置（允许前端所有域名访问）
const CORS_OPTIONS = {
...
app.post('/api/recognize/dish', async (req, res) => {
  try {
    ...
    // 获取有效AccessToken
const token = '24.3f7931c6f0fb1897aac6f8c4d2ecf3b5.2592000.1773714229.282335-122092421';
    // 调用百度菜品识别API
    const response = await axios.post(
...
app.post('/api/recognize/ingredient', async (req, res) => {
  try {
    ...
    // 获取有效AccessToken
const token = '24.4a12c5367be42a247e90501eac6c6c1b.2592000.1774343866.282335-122124389';
    // 调用百度食材识别组合API
    const response = await axios.post(
